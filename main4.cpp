...
#include <cstdlib>
#include <vector>
#include <algorithm> // for reverse (fake hash)
using namespace std;
...
private:
    ...
    string hashPassword(const string& password) {
        string fakeHash = password;
        reverse(fakeHash.begin(), fakeHash.end());
        return fakeHash;
    }

    bool kiemTraMatKhauGoc(const string& password, const string& hashed) {
...
public:
        ...
        for (auto& acc : danhsachtaikhoan) {
            if (acc.username == user) {
                ...
                if (xacThuc) {
                    ...
                    acc.hashpassword = acc.hashPassword(newPassword);
    ...
    static void doiMatKhau() {
        ...
        for (auto& acc : danhsachtaikhoan) {
            if (acc.username == user && acc.kiemTraMatKhauGoc(oldPassword, acc.hashpassword)) {
        ...
        cout << "Thong tin khong dung. Khong the cap nhat mat khau.\n";
    }
    ...
    bool kiemtramatkhau(const string& password) {
        return kiemTraMatKhauGoc(password, hashpassword);
    }
